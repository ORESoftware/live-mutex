 ...
 *
 * This version randomizes the order of test cases for each run and logs the sequence
 */
...
const createClients = async (count: number, options = {}): Promise<Client[]> => {
    ...
    const clientPromises = Array.from({ length: count }).map(async () => {
        ...
        client.emitter.on('warning', v => {
            // Silence warnings during tests to keep logs cleaner
        });
...
// Define test cases as functions
const testCases = [
    { name: "Multiple clients competing for the same lock", fn: testMultipleClientsForSameLock },
    { name: "Semaphore with multiple holders", fn: testSemaphoreWithMultipleHolders },
    { name: "Concurrent lock and unlock operations", fn: testConcurrentLockAndUnlock },
    { name: "Lock expiration", fn: testLockExpiration },
    { name: "Client disconnection", fn: testClientDisconnection },
    { name: "Forced unlocks", fn: testForcedUnlocks },
    { name: "Rapid lock/unlock cycles", fn: testRapidLockUnlockCycles },
    { name: "Promise-based API for lock/unlock", fn: testPromiseBasedAPI },
    { name: "Cascading locks", fn: testCascadingLocks },
    { name: "Lock contention with backpressure", fn: testLockContentionWithBackpressure },
    { name: "TTL variations", fn: testTTLVariations },
    { name: "System stability under load", fn: testSystemStabilityUnderLoad },
    { name: "Lock reacquisition", fn: testLockReacquisition },
    { name: "Staggered client connections and disconnections", fn: testStaggeredClientConnections },
    { name: "Ping/connectivity tests", fn: testPingConnectivity }
];
...
// Main function to run all tests in random order
async function runRandomizedTests(iterations: number = 20) {
    console.log(`Starting Randomized Live Mutex Tests (${iterations} iterations)`);
    console.log(`Remote broker at ${REMOTE_HOST}:${REMOTE_PORT}`);
    ...
    for (let iteration = 1; iteration <= iterations; iteration++) {
        ...
        // Generate and log the random test order for this iteration
        const randomizedTests = shuffleArray(testCases);
        const testOrder = randomizedTests.map((t, i) => `${i+1}. ${t.name}`);

        console.log("Test execution order for this iteration:");
        testOrder.forEach(t => console.log(t));
        console.log("");

        // Collection for clients to be cleaned up
        const allClients: Client[] = [];

        try {
            // Run each test in the randomized order
            for (let i = 0; i < randomizedTests.length; i++) {
                const test = randomizedTests[i];
                console.log(`\nRunning test ${i+1}/${randomizedTests.length}: ${test.name}`);
                await test.fn(allClients);
            }

            console.log(`\n✓ Iteration ${iteration} completed successfully`);
        } catch (error) {
            // If there's an error, log the test sequence and exit
            console.error(`\n✗ Error in iteration ${iteration}:`, error);

            // Log the test order that produced the error
            const logFile = `mutex-test-error-sequence-${Date.now()}.log`;
            const logContent = [
                `Error occurred in iteration ${iteration}:`,
                error.toString(),
                "\nTest execution order that produced the error:",
                ...testOrder,
            ].join('\n');

            try {
                fs.writeFileSync(logFile, logContent);
                console.error(`Test sequence logged to ${logFile}`);
            } catch (fsError) {
                console.error(`Failed to write error log: ${fsError}`);
                console.error(`Test sequence that produced the error:`, testOrder);
            }
    ...
    console.log("\n==========================================");
    console.log(`✓ All ${iterations} test iterations completed successfully`);
    console.log("==========================================");
}
...
async function testMultipleClientsForSameLock(allClients: Client[]) {
    ...
    console.log(`✓ Test passed: All ${acquiredCount} clients acquired and released the lock`);
}

// Test 2: Semaphore with multiple holders
async function testSemaphoreWithMultipleHolders(allClients: Client[]) {
    console.log('\nTest: Semaphore with multiple holders');
    ...
    console.log(`✓ Test passed: Max concurrent holders: ${maxConcurrentHolders}, limit was ${MAX_HOLDERS}`);
}
...
async function testConcurrentLockAndUnlock(allClients: Client[]) {
    ...
    console.log(`✓ Test passed: ${completedCount} operations completed successfully`);
}
...
async function testLockExpiration(allClients: Client[]) {
    ...
    console.log('✓ Test passed: Lock properly expired and was acquired by second client');
}
...
async function testClientDisconnection(allClients: Client[]) {
    ...
    console.log('✓ Test passed: Lock properly released after client disconnection');
}
...
async function testForcedUnlocks(allClients: Client[]) {
    ...
    console.log('✓ Test passed: Force unlock worked correctly');
}
...
async function testRapidLockUnlockCycles(allClients: Client[]) {
    ...
    console.log(`✓ Test passed: Completed ${completedCycles}/${cycles} rapid lock/unlock cycles`);
}

// Test 8: Promise-based API for lock/unlock
async function testPromiseBasedAPI(allClients: Client[]) {
    console.log('\nTest: Promise-based API for lock/unlock');
    ...
    console.log(`✓ Test passed: All ${successCount} clients successfully used promise-based API`);
}

// Test 9: Cascading locks (lock multiple resources in sequence)
async function testCascadingLocks(allClients: Client[]) {
    console.log('\nTest: Cascading locks (acquiring multiple locks in sequence)');
    ...
    console.log(`✓ Test passed: ${successCount}/${clientCount} clients successfully acquired cascading locks`);
}

// Test 10: Lock contention with backpressure
async function testLockContentionWithBackpressure(allClients: Client[]) {
    console.log('\nTest: Lock contention with backpressure');
    ...
    console.log(`✓ Test passed: ${successCount}/${clientCount} clients succeeded with max queue size of ${maxQueuedRequests}`);
}
...
async function testTTLVariations(allClients: Client[]) {
    ...
    console.log(`✓ Test passed: All ${results.length} TTL variations correctly expired`);
}
...
async function testSystemStabilityUnderLoad(allClients: Client[]) {
    ...
    console.log(`✓ Test passed: System remained stable under load with ${successRate.toFixed(2)}% success rate`);
}

// Test 13: Lock reacquisition
async function testLockReacquisition(allClients: Client[]) {
    console.log('\nTest: Lock reacquisition');
    ...
    console.log(`✓ Test passed: Successfully reacquired the same lock ${successCount} times`);
}

// Test 14: Staggered client connections and disconnections
async function testStaggeredClientConnections(allClients: Client[]) {
    console.log('\nTest: Staggered client connections and disconnections');
    ...
    console.log(`✓ Test passed: Successfully managed staggered client connections and disconnections`);
}

// Test 15: Ping/connectivity tests
async function testPingConnectivity(allClients: Client[]) {
    console.log('\nTest: Ping/connectivity tests');
    ...
    console.log(`✓ Test passed: Successfully completed ping connectivity tests`);
}

// Main function to run the tests
if (require.main === module) {
    runRandomizedTests(20).catch(err => {
