...
async function test1_DefaultMaxIsOne(broker: Broker1, clients: Client[]): Promise<TestResult> {
    ...
    try {
        ...
        const promises = clients.slice(0, 5).map((client, index) => 
...
async function test2_SemaphoreMaxThree(broker: Broker1, clients: Client[]): Promise<TestResult> {
    ...
    try {
        ...
        const promises = clients.slice(0, 10).map((client, index) => 
            new Promise<void>((resolve) => {
...
async function test3_SemaphoreMaxTen(broker: Broker1, clients: Client[]): Promise<TestResult> {
    ...
    try {
        ...
        const promises = clients.slice(0, 20).map((client, index) => 
            new Promise<void>((resolve) => {
                client.lock(key, { max: MAX_HOLDERS }, (err, unlock) => {
                    if (err) {
                        log(`Client ${index} failed:`, err.message);
...
async function runTests(): Promise<void> {
    ...
    try {
        ...
        for (let i = 0; i < 25; i++) {
            const client = new Client({ port: PORT, lockRequestTimeout: 30000 });
            await client.ensure();
            clients.push(client);
        }
