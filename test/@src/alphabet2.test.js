...
Test.create(['lmUtils', (b, assert, before, describe, it, path, fs, inject, after) => {
        ...
        inject(j => {
            ...
            j.register('broker', new broker_1_1.Broker1(brokerConf).ensure().then(handleEvents));
        });
        describe('post', function (b) {
            ...
            it.cb('all', { timeout: 50000 }, t => {
                async.times(5, function (n, cb) {
                    const c = new client_1.Client({ port: conf.port, ttl: 5000, lockRequestTimeout: 100 });
                    handleEvents(c);
                    ...
                    t.done(new Error('original different than result' + result));
                });
            });
        });
    }]);
