...
Test.create({ mode: 'series' }, ['Client', 'lmUtils', 'Promise', function (b, assert) {
        ...
        before(function () {
            ...
            return new live_mutex_1.Broker1(brokerConf).start();
        });
        it.cb('yes - 1', { timeout: 3900 }, t => {
            const c = live_mutex_1.Client.create(conf);
            c.ensure((err, c) => {
                if (err) {
                    return t.fail(err);
                }
        ...
        it.cb('yes - 2', { timeout: 3900 }, t => {
            const c = new live_mutex_1.Client(conf);
        ...
        it.cb('yes - 4', { timeout: 3500 }, t => {
            return live_mutex_1.Client.create(conf).ensure().then(c => {
