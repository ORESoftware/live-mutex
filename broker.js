    ...
    function Broker($opts, cb) {
        ...
        this.ensure = function ($cb) {
            ...
            if ($cb) {
                ...
                var to_1 = setTimeout(cb_1.bind(this, 'err:timeout'), 3000);
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    var to = setTimeout(reject.bind(null, 'err:timeout'), 3000);
                    ee.once('open', function () {
                        clearTimeout(to);
                        resolve(_this);
                    });
        ...
        wss.on('connection', function (ws) {
            ...
            if (!ws.wsClientId) {
                ...
                var a = _this.clientIdsToKeys[ws.wsClientId] = _this.clientIdsToKeys[ws.wsClientId] || {};
                a.ws = a.ws || ws;
                a.keys = a.keys || [];
            }
            ws.on('close', function () {
                ...
                var keys;
                if (keys = _this.wsLock[ws.wsClientId]) {
                    keys.forEach(function (k) {
                        removeWsLockKey(_this, ws, k);
                        var lck;
                        if (lck = _this.locks[k]) {
                            _this.unlock({
                                force: true,
                                key: k
                            }, ws);
            ...
            ws.on('message', function (msg) {
                ijson.parse(msg).then(function (data) {
                    ...
                    if (data.type === 'unlock') {
                        ...
                        _this.bookkeeping.keys[data.key].lockCount++;
                        ...
                        _this.bookkeeping.keys[key_1].unlockCount++;
                        debug('\n', ' => Lock/unlock count (broker), key => ', '"' + key_1 + '"', '\n', util.inspect(_this.bookkeeping.keys[key_1]), '\n');
                    }
...
Broker.prototype.sendStatsMessageToAllClients = function () {
    var _this = this;
    var time = Date.now();
    var clients = Object.keys(this.clientIdsToKeys);
    async.mapSeries(clients, function (k, cb) {
        var obj = _this.clientIdsToKeys[k];
        var keys = obj.keys;
        var ws = obj.ws;
...
Broker.prototype.unlock = function _unlock(data, ws) {
    ...
    if (lck && (same || force)) {
        ...
        Object.keys(this.wsLock).forEach(function (k) {
            var keys = _this.wsLock[k];
            if (keys) {
        ...
        console.error(colors.red.bold(' => Usage / implementation error => this should not happen => no lock with key => '), colors.red('"' + key + '"'));
        Object.keys(this.wsLock).forEach(function (k) {
            var keys = _this.wsLock[k];
            if (keys) {
                var i = keys.indexOf[key];
